...
回调中必须先请求画中画，再向后台核对活跃标签页；若后台未确认切标签页，立即关闭本次浮窗（可能短暂闪现）。快速返回或主动退出时，也会取消尚未完成的进入请求。用户关闭浮窗后，本轮隐藏期间禁止自动重开，直到真正返回页面再切走。退出失败时，面板会显示浏览器的错误名称，不再报告成功。普通路径的异步后台回复带有可见性版本检查，过期回复不会再打开浮窗。
...
4. 分别用视频右键菜单、浮窗关闭按钮、插件面板退出，确认窗口关闭且本轮不会立刻重新弹出；返回后再次切走仍可自动进入。
5. 检查快速切走又切回不会留下浮窗；进入尚未完成时点击插件切换按钮，应取消进入；暂停视频或关闭总开关后不会自动浮窗。
6. 检查切应用/最小化时不会保留扩展自动打开的浮窗。
